...
public interface NodeRepository extends RestRepository<Node, String> {
	 ...
	 */
	String VISIBLE_NODES_PART = "(" + SystemUser.IS_ADMIN + " OR EXISTS(SELECT 1 FROM DelegateNode d WHERE "
			+ MATCH_DELEGATE;
	...
	/**
	 * Visible nodes condition for update : either administrator, either can edit it, either administer it.
	 */
	String WRITE_NODES = VISIBLE_NODES_PART + " AND (d.canWrite = true OR d.canAdmin = true)))";

	/**
	 * Visible nodes condition for deletion : either administrator, either administer it.
	 */
	...
	/**
	 * Return a {@link Node} by its identifier if it is visible and writable for the current user.
	 *
	 * @param id   The identifier to find.
	 * @param user The user requesting the node.
	 * @return The visible node or <code>null</code>.
	...
	/**
	 * Return a {@link Node} by its identifier if it is visible and administered for the current user.
	 *
	 * @param id   The identifier to find.
	 * @param user The user requesting the node.
	 * @return The visible node or <code>null</code>.
	...
	@Query("SELECT n FROM Node n LEFT JOIN n.refined nr1 LEFT JOIN nr1.refined nr2"
			+ " WHERE (CAST(:parent as string) IS NULL OR (:parent = 'service' AND n.refined IS NULL) OR n.refined.id = :parent)"
			+ " AND (:depth < 0 OR :depth > 1 OR (:depth = 0 AND nr1 IS NULL) OR (:depth = 1 AND nr2 IS NULL))  "
			+ " AND (CAST(:mode as string) IS NULL OR n.mode = :mode OR n.mode = org.ligoj.app.api.SubscriptionMode.ALL)        "
			+ " AND (UPPER(n.name) LIKE UPPER(CONCAT(CONCAT('%',:criteria),'%'))) AND " + VISIBLE_NODES
			+ " ORDER BY n.id")
	Page<Node> findAllVisible(String user, String criteria, String parent, SubscriptionMode mode, int depth,
			Pageable page);
